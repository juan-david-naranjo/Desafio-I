...
bool archivosiguales(string archivo1,string archivo2){
    /*
     *funcion que recibira los archivos txt de salida, luego de cada proceso de destransformacion
     y compara si son iguales o no
    *como los archivos de salida unicamente contiene la suma de los pixeles, es decir unicamente contendra numeros enteros,
    solo necesitaremos verificar que los valores linea por linea de archivo 1 sean iguales a archivo 2
    */
    ifstream arch(archivo1);        //abre los archivos en modo lectura
    ifstream arch2(archivo2);
    int numeros1,numeros2;
    if(!arch.is_open()){cout<<"el archivo 1 no se pudo abrir"<<endl;}       //verifica si estan abiertos correctamente
    if(!arch2.is_open()){cout<<"el archivo 2 no se pudo abrir"<<endl;}

    while(arch>>numeros1 && arch2>>numeros2){
        if(numeros1!=numeros2){
            cout<<"los archivos de entrada  no son iguales"<<endl;
            arch.close();
            arch2.close();
            return false;
        }
    ...
    cout<<"los archivos de entrada son iguales "<<endl;
    return true;
}
...
/*
*este agoritmo recibe un arreglo de unsigned char , luego itera en cada objeto del arreglo y
*le desplaza un bit a la derecha a dicho elemento.
*/
...
void XoR(unsigned char* i_D, unsigned char* i_M,int width, int height){
    /*
    *este algoritmo recibe la i_D y la i_M y hace operacion XoR, pixel por pixel de la imagen
    *retornando asi el resultado que es una imagen i_D XoR i_M
